...
def test_ear_trainer_is_served_on_the_books_host(books_client):
    page = books_client.get("/ear/")
    assert page.status_code == 200
    assert "Say the Interval" in page.text


def test_ear_bare_path_redirects_to_the_directory(books_client):
    r = books_client.get("/ear", follow_redirects=False)
    assert r.status_code in (307, 308)
    assert r.headers["location"].endswith("/ear/")


def test_ear_manifest_is_served_as_a_manifest(books_client):
    """Chrome refuses to offer installation if this arrives as octet-stream."""
    r = books_client.get("/ear/manifest.webmanifest")
    assert r.status_code == 200
    assert r.headers["content-type"].startswith("application/manifest+json")
    assert r.json()["start_url"] == "./"


def test_ear_service_worker_is_reachable_at_the_scope_root(books_client):
    """The worker can only control /ear/ if it is served from /ear/."""
    r = books_client.get("/ear/sw.js")
    assert r.status_code == 200
    assert "javascript" in r.headers["content-type"]


def test_ear_assets_are_served(books_client):
    for path in ("/ear/app.js", "/ear/audio.js", "/ear/intervals.js",
                 "/ear/style.css", "/ear/icons/icon-512.png"):
        assert books_client.get(path).status_code == 200, path


def test_ear_trainer_is_also_on_the_other_hostname(client):
    assert client.get("/ear/").status_code == 200


def test_mounting_ear_did_not_expose_kite_routes_on_the_books_host(books_client):
